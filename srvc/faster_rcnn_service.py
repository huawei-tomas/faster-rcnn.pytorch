...
def _collect_regions(model, im_data, gt_boxes, mode="align"):
    """Collect features from ground truth regions.
    Arguments:
      model (class): class containing fasterRCNN algorithm in nn.Sequential form.
      im_data (torch.FloatTensor): image data
      gt_boxes (torch.FloatTensor): ground truth bounding boxes
      mode (string): pooling mode to use creating region features ("align" or
                     "pooling")
    Returns:
      pooled_feat (np.array): array with numerical features for the ground
                              truth regions.
    """
...
def _process_images_infer(image_dir, imagelist, model):
    """Run model on all images in imagelist.
    Arguments:
      image_dir (string): directory containing input images.
      imagelist (list): list of image locations.
      model (class): class containing fasterRCNN algorithm in nn.Sequential form.
     Returns:
      res (list): a list of dictionaries with rois, cls_prob, and other info for
                  each input image.
    """
    res = []
...
def _main(serialize=False):
    ...
    Arguments:
      serialize (bool): a flag that indicates whether to serialize the output.
    Returns:
      res (list): a list of dictionaries with rois, cls_prob, and other info for
                  each input image.
    """
